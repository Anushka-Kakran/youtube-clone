  ...
  const fetchMyVideos = useCallback(async () => {
    try {
      const res = await axios.get(
        "https://youtube-clone-zd16.onrender.com/video/get-video",
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      // Filter videos to only show those belonging to the logged-in user
      const myVideos = (res.data.videos || []).filter(
        (v) => v.user_id?._id === currentUserId,
      );
      setVideos(myVideos);
    } catch (err) {
      console.error(err);
        ...
        <div className="max-w-6xl mx-auto px-4 flex flex-col md:flex-row items-center md:items-end -mt-12 md:-mt-16 gap-6 pb-6 border-b dark:border-white/10">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-gray-500 text-sm">
              @{videos[0]?.user_id?.channelName?.replace(/\s/g, "")} •{" "}
          ...
          <div key={video._id} className="relative group">
            ...
            <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <button
                ...
                className="bg-blue-600 text-white p-2 rounded-full shadow-lg hover:scale-110"
              >
