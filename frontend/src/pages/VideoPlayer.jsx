...
export default function VideoPlayer() {
  ...
  const userLogo = localStorage.getItem("logo");
  ...
  const fetchComments = async () => {
    try {
      const res = await axios.get(`https://youtube-clone-zd16.onrender.com/comment/${id}`);
 ...
 const handleLike = async () => {
  ...
  setVideo((prev) => ({
    ...
    dislike: prev.dislikedBy?.includes(currentUserId) ? Math.max(0, prev.dislike - 1) : prev.dislike,
    likedBy: [...(prev.likedBy || []), currentUserId],
    dislikedBy: prev.dislikedBy?.filter(uid => uid !== currentUserId)
  }));

  try {
    await axios.put(`https://youtube-clone-zd16.onrender.com/video/like/${id}`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (e) {
...
const handleDislike = async () => {
  ...
  setVideo((prev) => ({
    ...
    likes: prev.likedBy?.includes(currentUserId) ? Math.max(0, prev.likes - 1) : prev.likes,
    dislikedBy: [...(prev.dislikedBy || []), currentUserId],
    likedBy: prev.likedBy?.filter(uid => uid !== currentUserId)
  }));

  try {
    await axios.put(`https://youtube-clone-zd16.onrender.com/video/dislike/${id}`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
  } catch (e) {
    fetchData(); // Rollback on error
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      await axios.post(`https://youtube-clone-zd16.onrender.com/comment/new-comment/${id}`, 
        { commentText }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setCommentText("");
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      await axios.delete(`https://youtube-clone-zd16.onrender.com/comment/${commentId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchComments();
    } catch (err) { console.error(err); }
  ...
  const handleUpdateComment = async (commentId) => {
    try {
      await axios.put(`https://youtube-clone-zd16.onrender.com/comment/${commentId}`, 
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center justify-between mt-3 gap-4 border-b pb-4">
            <div className="flex items-center gap-3">
              ...
              />
              <div className="mr-6">
                <p className="font-bold text-base">{video.user_id?.channelName}</p>
              ...
              <button
                ...
                className={`px-6 py-2 rounded-full font-bold text-sm transition-all ${
                  isSubscribed ? "bg-zinc-200 text-black" : "bg-black text-white"
                }`}
              >
  ...
  <button 
    ...
    className={`flex items-center gap-2 px-5 hover:bg-zinc-200 border-r border-zinc-300 transition-colors ${
      video?.likedBy?.includes(currentUserId) ? "text-blue-600" : "text-black"
    }`}
  >
  ...
  <button 
    ...
    className={`flex items-center gap-2 px-5 hover:bg-zinc-200 transition-colors ${
      video?.dislikedBy?.includes(currentUserId) ? "text-red-600" : "text-black"
    }`}
  >
    👎 <span className="font-bold text-sm">{video.dislike || 0}</span>
          ...
          <div className="bg-zinc-100 p-3 rounded-xl mt-4 text-sm">
            <p className="font-bold">{video.views || 0} views • {formatTimeAgo(video.createdAt)}</p>
            <p className="text-zinc-700 mt-1 whitespace-pre-line">
              {isExpanded ? video?.description : video?.description?.split(" ")?.slice(0, 30).join(" ")}
              {video?.description?.split(" ")?.length > 30 && (
                <span onClick={() => setExpanded(!isExpanded)} className="ml-2 text-blue-600 cursor-pointer font-semibold">
          ...
          <div className="mt-6">
            ...
            <div className="flex gap-4 mb-8">
              ...
              <img src={userLogo} className="w-10 h-10 rounded-full shrink-0" alt="me" />
              <div className="flex-1">
                <input
                  type="text"
                ...
                <div className="flex justify-end mt-2">
                  <button onClick={handleCommentSubmit} className="bg-zinc-900 text-white px-4 py-2 rounded-full text-sm font-bold">
                    Comment
                  </button>
                </div>
              </div>
            ...
            <div className="space-y-6">
              {allComments.map((c) => (
                <div key={c._id} className="flex gap-3 items-start group py-2">
                  <img 
                    src={c.userId?.logoUrl} 
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <p className="text-xs font-bold">@{c.userId?.channelName}</p>
                        <span className="text-[10px] text-zinc-500">{formatTimeAgo(c.createdAt)}</span>
                      </div>

                      {/* Fixed: Using currentUserId for ownership check */}
                      {currentUserId && String(c.userId?._id) === String(currentUserId) && (
                        <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button 
                            onClick={() => { setEditingCommentId(c._id); setEditValue(c.commentText); }} 
                    ...
                    {editingCommentId === c._id ? (
                      <div className="mt-2">
                        ...
                        <div className="flex gap-4 mt-2">
                          <button onClick={() => handleUpdateComment(c._id)} className="text-xs font-bold text-green-600">SAVE</button>
                          <button onClick={() => setEditingCommentId(null)} className="text-xs font-bold text-gray-500">CANCEL</button>
                        </div>
                      </div>
                    ) : (
                      <p className="text-sm mt-1 text-zinc-800">{c.commentText}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="w-full lg:w-[400px]">
          <div className="border border-zinc-200 rounded-xl bg-white overflow-hidden shadow-sm">
            <div className="p-3 border-b bg-zinc-50 font-bold text-sm">Next Videos</div>
            <div className="max-h-[600px] overflow-y-auto">
              {playlist.map((item, index) => (
                <div key={item._id} className={`flex gap-2 p-2 cursor-pointer hover:bg-zinc-100 ${item._id === id ? "bg-zinc-100" : ""}`} onClick={() => navigate(`/video/${item._id}`)}>
                  <span className="text-[10px] text-zinc-400 self-center w-4 text-center">{index + 1}</span>
                  <img src={item.thumbnailUrl} className="w-28 h-16 rounded-lg object-cover" alt="thumb" />
                  <div className="overflow-hidden">
                    <p className="text-xs font-bold line-clamp-2 leading-tight">{item.title}</p>
                    <p className="text-[10px] text-zinc-500 mt-1">{item.user_id?.channelName}</p>
                  </div>
                </div>
              ))}
            </div>
